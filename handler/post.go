...
func uploadHandler(w http.ResponseWriter, r *http.Request) {
    ...
    //.() equals to cast changing to corresponding format automatically
    ...
    //hashmap: mediaTypes[suffix] check if contains(key) suffix return ok 
...
func searchHandler(w http.ResponseWriter, r *http.Request) {//http.ResponseWriter is an interface
    fmt.Println("Received one request for search")
    w.Header().Set("Content-Type", "application/json")

    user := r.URL.Query().Get("user")//r represents request
    ...
    js, err := json.Marshal(posts)//if no error, return search result in json format
    if err != nil {
        http.Error(w, "Failed to parse posts into JSON format", http.StatusInternalServerError)
        //http.StatusInternalServerError is 500, a constant error
        fmt.Printf("Failed to parse posts into JSON format %v.\n", err)
        return
    }
    w.Header().Set("Content-type", "application/json")
    //tell frontend, the return result in json format
    w.Write(js)
}
